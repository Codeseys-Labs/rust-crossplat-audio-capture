...
fn configure_linux_build() {
    ...
    eprintln!("Missing system libraries for application capture: {}", missing.join(", "));
    ...
    eprintln!("For Ubuntu/Debian: sudo apt-get install libpipewire-0.3-dev pkg-config build-essential");
    process::exit(1);
}
        ...
        if let Ok(version_str) = String::from_utf8(version.stdout) {
            ...
            if let Some((major, minor)) = parse_macos_version(&version_str) {
                if major < 14 || (major == 14 && minor < 4) {
                    println!("cargo:warning=Process Tap APIs require macOS 14.4+, current: {}", version_str);
