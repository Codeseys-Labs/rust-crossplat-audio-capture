...
|---|---|---|
| **Device — default mic** (`Device("default")`) | 🟢 **emulator-verified** (PR #66, CI run 29680252107: instrumented androidTest, real app uid + RECORD_AUDIO, drove the shipped rsac-ffi C ABI on an API 30 x86_64 AVD — 3 buffers / 2193 frames @ 48 kHz mono delivered; real-device verification still open, rsac-e6d3) | 🟢 **simulator-verified** (PR #67, CI run 29717781236: BlackHole virtual input as the host default before sim boot gave the sim a real mic route — bare public-API smoke AND app-hosted TCC harness via the C ABI each delivered 3 buffers / 14400 frames @ 48 kHz stereo; real-device verification still open, rsac-97c8) |
| **System default** (= playback capture, ADR-0013) | 🟢 **emulator-verified, SystemDefault only** (PR #67, CI run 29717781237: appops-pre-granted consent drove the shipped `RsacProjection` flow — real token minted via consent→FGS→`getMediaProjection`, `AudioPlaybackCapture` delivered 3 buffers / 1440 frames @ 48 kHz stereo of the test's own MEDIA tone through the public API; UID-filtered tiers + real device still open) | 🟡 compiled, unverified (rsac-b3aa: ReplayKit ring consumer; needs `with_ios_app_group` + embedded extension + user-started broadcast) |
| **Application / ByName / ProcessTree** | 🟢 **emulator-verified, filter plumbing via SELF-capture** (PR #68, CI run 29727707513: each tier — own uid / own package / own pid — resolved through `resolve_match_uid` → `addMatchingUid` and delivered 3 buffers / 1440 frames @ 48 kHz stereo with a fresh consent token per tier; capturing a **different** app's audio stays unverified) | ❌ permanent — no iOS API (never soften) |
| **Device selection (real device list)** | 🟢 **enumeration emulator-verified** (PR #68, run 29727707513: 6 devices — 4 real numeric-id inputs + default + playback-capture sentinels — via `ActivityThread.currentApplication()`; real-input presence proxies the `android_device_enumeration_available` gate). `Device(real-id)` **targeted capture** stays 🟡 compiled-unverified (needs a token-free driver export — follow-up on rsac-ad8a's close notes) | ❌ session-routed, not free selection |
...
**Blacksmith audio device probe results** (run 2026-04-13; Windows re-verified live 2026-07-17, run 29545897122, rootfs `windows25-full-x64-251112`):
...
| **Windows** | ❌ No audio stack (**SKU-structural**) | **Root cause (2026-07-17 re-probe): the image is Windows Server 2025 build 26100, `WindowsInstallationType: Server Core`** — Server Core strips the desktop audio stack *by design*, independent of Firecracker; image updates alone won't close the gap. Confirmed still absent: `AudioSrv`/`AudioEndpointBuilder` services, `AudioSes.dll`/`AudioEndpointBuilder.dll`/`Audiodg.exe`/`AudioEng.dll` in System32, `MMDevices\Audio\Render|Capture` registry keys (`MMDevAPI.dll` *is* present — an API shim with nothing behind it). VB-CABLE (LABSN) installs "successfully" and its **PnP device node now registers** (`Win32_SoundDevice`/`Get-PnpDevice` show "VB-Audio Virtual Cable") **but with `Status: Error`** — enumerable, never functional; `Get-AudioDevice -List` returns empty. `Get-WindowsFeature Server-Media-Foundation` now reports `Available` (disabled, not "unavailable" as the 2026-04 probe recorded) — untested whether installing it provisions `AudioSrv`; do not assume it's a recovery path. Windows audio integration tests run on GitHub-hosted `windows-latest` instead. **Probe-script gotcha:** broad `Get-Service` (no `-Name` filter) hard-aborts the pwsh pipeline on this image (`MDCoreSvc` PermissionDenied) — filter by name or wrap in try/catch. |
