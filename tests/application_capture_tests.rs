...
use rsac::audio::{
    ...
    ApplicationCapture, ApplicationCaptureFactory,
};
...
fn test_list_applications_returns_results() {
    ...
    assert!(
        result.is_ok(),
        "Listing applications should not fail: {:?}",
        result.err()
    );
    ...
    for app in applications.iter().take(5) {
...
fn test_invalid_process_id_handling() {
    ...
    for pid in invalid_pids {
        ...
        assert!(
            result.is_err(),
            "Capturing from invalid PID {} should fail",
            pid
        );
    }
}
...
fn test_nonexistent_application_name() {
    ...
    for name in fake_names {
        ...
        assert!(
            result.is_err(),
            "Capturing from fake app '{}' should fail",
            name
        );
    }
}
    ...
    if let Some(app) = applications.first() {
        if let Ok(mut capture) = capture_application_by_pid(app.process_id) {
            ...
            if start_result.is_ok() {
                ...
                println!(
                    "Could not start capture for PID {}: {:?}",
                    app.process_id,
                    start_result.err()
                );
            }
    ...
    if let Some(app) = applications.first() {
        if let Ok(mut capture) = capture_application_by_pid(app.process_id) {
            ...
            if start_result.is_ok() {
                ...
                assert!(
                    !capture.is_capturing(),
                    "Should not be capturing after multiple stops"
                );
            }
        }
    }
...
fn test_platform_specific_info() {
    ...
    for app in applications.iter().take(3) {
        match &app.platform_specific {
            ...
            #[cfg(target_os = "linux")]
            rsac::audio::PlatformSpecificInfo::Linux {
                node_id,
                media_class,
            } => {
                println!(
                    "Linux app: {} (node: {:?}, class: {:?})",
                    app.name, node_id, media_class
                );
                assert!(
                    node_id.is_some(),
                    "Node ID should be present for Linux apps"
                );
            }
...
fn test_concurrent_capture_attempts() {
    ...
    if let Some(app) = applications.first() {
        ...
        println!(
            "Concurrent capture attempts: {} succeeded out of 3",
            success_count
        );
    }
}
