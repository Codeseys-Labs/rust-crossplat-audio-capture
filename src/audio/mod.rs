...
#[cfg(target_os = "linux")]
pub use linux::{PulseAudioBackend, PipeWireBackend};
#[cfg(target_os = "macos")]
...
pub fn get_audio_backend() -> Result<Box<dyn AudioCaptureBackend>, AudioError> {
    ...
    #[cfg(target_os = "linux")]
    {
        if linux::PulseAudioBackend::is_available() {
            ...
            Err(AudioError::BackendUnavailable("No supported audio backend available"))
        }
    }
    #[cfg(target_os = "macos")]
    {
    ...
    {
        Err(AudioError::BackendUnavailable("Unsupported operating system"))
    }
}
