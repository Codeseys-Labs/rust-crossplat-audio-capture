...
fn test_pipewire_enumeration(firefox_pids: &[u32]) {
        ...
        .output()
    {
        if output.status.success() {
            ...
            println!("    📊 Found {} Firefox references in PipeWire dump", firefox_count);
        ...
        .output()
    {
        if output.status.success() {
            ...
            println!("    📋 Found {} Firefox-related nodes via pw-cli", firefox_nodes.len());
...
fn test_firefox_capture(firefox_pids: &[u32]) {
    ...
    println!("    • Looking for PipeWire nodes for PID {}", main_firefox_pid);
    ...
    if let Some(node_info) = find_audio_node_for_pid(main_firefox_pid) {
        ...
        println!("    ❌ No audio node found for Firefox PID {}", main_firefox_pid);
        ...
        if output.status.success() {
            ...
            for line in lines {
                ...
                if line.contains("type PipeWire:Interface:Node") {
                    ...
                    if let Some(id_start) = line.find("id ") {
                        if let Some(comma) = line[id_start + 3..].find(',') {
                            if let Ok(id) = line[id_start + 3..id_start + 3 + comma].trim().parse::<u32>() {
                ...
                // Check for application name
                    ...
                    println!("      📍 Found Firefox application.name in node {:?}", current_node_id);
                }
                // Check for process ID
                else if line.contains("application.process.id") && line.contains(&pid_str) {
                    found_target_pid = true;
                    println!("      📍 Found target PID {} in node {:?}", pid, current_node_id);
                }
                // Check for media class
                    ...
                    println!("      📍 Found audio output stream in node {:?}", current_node_id);
                }

                // If we have all the pieces for this node, check if it matches
                if let (Some(node_id), Some(_), Some(_)) = (&current_node_id, &current_app_name, &current_media_class) {
                    if found_target_pid {
                                ...
                                if lines[k].contains("\"id\":") {
                                    if let Some(id_str) = extract_json_number_value(lines[k]) {
                                        if let Ok(node_id) = id_str.parse::<u32>() {
                                            return Some(format!("Node {} (Firefox Audio Stream via pw-dump)", node_id));
                                        }
                                    }
                                }
                            }
...
fn simulate_firefox_audio_capture() {
    ...
    while sample_count < max_samples {
        ...
        if sample_count % 10 == 0 {
            println!("      📊 Sample {}: Firefox audio level = {:.3}", sample_count, rms);
        }
    ...
    println!("    📈 Captured {} audio buffers from Firefox", sample_count);
}
