...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.verbose {
        ...
        println!("Looking for frequency: {} Hz (±{} Hz)", args.frequency, args.tolerance);
    ...
    let samples: Vec<f32> = match spec.sample_format {
        ...
        hound::SampleFormat::Int => {
            ...
            int_samples.into_iter().map(|s| s as f32 / max_val).collect()
        }
    };
    ...
    let analysis_result = analyze_frequency(&mono_samples, sample_rate, args.frequency, args.tolerance);
    
    match analysis_result {
        Some(detected_freq) => {
            println!("✅ SUCCESS: Detected frequency {} Hz (expected {} Hz)", detected_freq, args.frequency);
            
            // Check amplitude
            let max_amplitude = mono_samples.iter().map(|s| s.abs()).fold(0.0f32, f32::max);
            if max_amplitude >= args.amplitude_threshold {
                println!("✅ SUCCESS: Amplitude {} >= threshold {}", max_amplitude, args.amplitude_threshold);
            } else {
                println!("⚠️  WARNING: Low amplitude {} < threshold {}", max_amplitude, args.amplitude_threshold);
            }
        }
        None => {
            println!("❌ FAILURE: Expected frequency {} Hz not detected", args.frequency);
...
fn analyze_frequency(samples: &[f32], sample_rate: f32, target_freq: f32, tolerance: f32) -> Option<f32> {
    if samples.len() < 1024 {
        return None;
    }
