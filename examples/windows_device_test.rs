...
use rsac::audio::windows::{enumerate_application_audio_sessions, WindowsDeviceEnumerator};
...
fn test_device_enumeration() -> AudioResult<()> {
    ...
    if let Ok(Some(input_device)) =
        enumerator.get_default_device(rsac::core::interface::DeviceKind::Input)
    {
    ...
    if let Ok(Some(output_device)) =
        enumerator.get_default_device(rsac::core::interface::DeviceKind::Output)
    {
