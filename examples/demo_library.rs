...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "\n✅ Demo completed! Check {} for output files",
...
fn list_devices_and_applications(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "linux")]
    {
        match get_audio_backend() {
            Ok(backend) => match backend.list_applications() {
                Ok(applications) => {
                    for app in applications {
                        println!("  - {}", app.name);
                    }
...
fn test_application_capture(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match capture_application_audio(app_name, args.duration, &output_path, args.verbose) {
        ...
        Err(e) => {
            ...
            println!(
                "  💡 Make sure the application '{}' is running and playing audio",
                app_name
            );
        }
    }
...
#[cfg(target_os = "windows")]
fn capture_windows_application(
    app_name: &str,
    duration: u64,
    output_path: &PathBuf,
    verbose: bool,
    ...
    let target_session = sessions
        ...
        .ok_or_else(|| {
            AudioError::BackendSpecificError(format!("Application '{}' not found", app_name))
        })?;

    if verbose {
        println!(
            "    Found application: {} (PID: {})",
