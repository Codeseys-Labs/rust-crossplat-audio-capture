...
#[derive(Parser, Debug)]
#[command(about = "Test PipeWire audio capture functionality")]
struct Args {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    println!("Starting PipeWire audio capture test...");
    ...
    let app = apps
        .iter()
        .find(|app| app.executable_name.contains("test_tone"))
    ...
    if let Ok(mut child) = Command::new("pkill").args(["-f", "test_tone"]).spawn() {
        let _ = child.wait();
    }
