...
- PipeWire (now the only supported audio backend on Linux)
- Development libraries (for building from source):

  - `libpipewire-0.3-dev` - PipeWire development library
...
- Runtime libraries (for running the compiled binary):
  - `libpipewire-0.3-0` - PipeWire runtime library
