...
**Technical reason:** macOS gates Process Tap (the CoreAudio 14.4+ API rsac uses for per-process capture) behind the **Audio Capture** TCC service (`kTCCServiceAudioCapture`). This is a **distinct, stricter** TCC service from Screen Recording (`kTCCServiceScreenCapture`) — GH-hosted runners pre-grant Screen Recording to `/bin/bash` via `actions/runner-images`'s `configure-tccdb-macos.sh`, but do NOT pre-grant Audio Capture anywhere. Apps declare the dependency via the `NSAudioCaptureUsageDescription` key in `Info.plist` + matching entitlement; the OS prompts the user on first Process Tap attempt. The consent database is per-user, per-bundle-ID, and cannot be pre-seeded from a script without disabling SIP (which breaks a whole class of other system-integrity guarantees). A headless CI runner has no logged-in desktop session, no way to click the prompt, and no supported API to auto-approve. `tccutil reset` can *revoke* entries but cannot *grant* them. Reference implementation: [`insidegui/AudioCap`](https://github.com/insidegui/AudioCap) — its `AudioRecordingPermission.swift` + `Info.plist` show the canonical Audio Capture permission flow.

**Why this applies to every managed runner — not an rsac-specific constraint:** This is the macOS security model, not a rsac or GitHub limitation. The same wall hits **GitHub-hosted `macos-latest`**, **Blacksmith**, **BuildJet**, **Actuated**, and anyone else renting out managed macOS VMs. None of them can ship with Audio Capture pre-granted for an arbitrary test binary because TCC entries are bound to bundle ID + code signature + user, and the binary CI builds is freshly compiled each run. The only paths that *would* work — self-hosted macOS runners with a persistent logged-in user where you manually grant permission once — are out of scope for an OSS project's default CI.
...
**Manual QA workaround:** On a real macOS 14.4+ machine, grant the TCC **Audio Capture** permission to the terminal running cargo. Since there isn't a direct System-Settings UI panel for Audio Capture (it's triggered by first-prompt), the practical approach is: run the tests once interactively with `RSAC_CI_MACOS_TCC_GRANTED=1` — the OS will show a "Terminal.app would like to record audio" prompt on first Process Tap attempt. Grant it, fully quit and relaunch the terminal so the granted TCC entry is picked up at process start, then:

```sh
# On a real macOS 14.4+ machine, after granting Audio Capture via the
# interactive OS prompt (triggered on first rsac run), re-run with:
RSAC_CI_AUDIO_AVAILABLE=1 RSAC_CI_MACOS_TCC_GRANTED=1 \
  cargo test --test ci_audio \
  --no-default-features --features feat_macos \
...
**What this means for users:** Process Tap works correctly in production. End users grant the hosting application's **Audio Capture** permission through the standard macOS prompt on first capture attempt, after which per-process and process-tree capture function normally. The gap is strictly one of *continuous* verification: rsac cannot prove on every commit that Process Tap still works — it can only prove that it still *compiles* and that the non-Tap paths still work. Manual verification by a human on a real Mac before tagging a release is the correct mitigation, and is the same discipline every other macOS audio capture project (AudioCap, Loopback, Audio Hijack, Rogue Amoeba's SDK) applies.
